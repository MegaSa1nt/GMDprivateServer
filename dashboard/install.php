...
if(!$installed) {
	...
	// Add this section for the new table
	$check = $db->query("SHOW TABLES LIKE 'user_uploads'");
	$exist = $check->fetchAll();
	if(empty($exist)) $db->query("CREATE TABLE IF NOT EXISTS user_uploads (
		userID INT NOT NULL,
		lastUploadTime INT NOT NULL,
		PRIMARY KEY (userID)
	)");
