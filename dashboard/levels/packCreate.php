...
if(!empty($_POST["packName"])) {
	...
	$name = ExploitPatch::charclean($_POST["packName"]);
	$color = ExploitPatch::numbercolon($dl->hex2RGB($_POST["color"], true));
	$stars = ExploitPatch::number($_POST["stars"]);
	$coins = ExploitPatch::number($_POST["coins"]);
	if(!is_numeric($_POST['level_1']) OR !is_numeric($_POST['level_2']) OR !is_numeric($_POST['level_3']) OR $stars > 10 OR $stars < 0 OR $coins > 2 OR $coins < 0 OR !$gs->getLevelName(ExploitPatch::number($_POST['level_1'])) OR !$gs->getLevelName(ExploitPatch::number($_POST['level_2'])) OR !$gs->getLevelName(ExploitPatch::number($_POST['level_3']))) {
	$dl->printSong('<div class="form">
			<h1>'.$dl->getLocalizedString("errorGeneric").'</h1>
			<form class="form__inner" method="post" action="">
	...
	$query->execute([':name' => $name, ':levels' => $levels, ':stars' => $stars, ':coins' => $coins, ':diff' => $diff, ':rgb' => $color, ':c2' => ExploitPatch::number(str_replace("#", '', $_POST["color"])), ':time' => time()]);
