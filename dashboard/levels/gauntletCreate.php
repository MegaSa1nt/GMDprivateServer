...
if($_POST["checkbox_data"] == 'on') {
	...
	if(($_POST['gauntlet_id'] & !is_numeric($_POST['gauntlet_id'])) OR !is_numeric($_POST['level_1']) OR !is_numeric($_POST['level_2']) OR !is_numeric($_POST['level_3']) OR !is_numeric($_POST['level_4']) OR !is_numeric($_POST['level_5'])) {
	$dl->printSong('<div class="form">
			<h1>'.$dl->getLocalizedString("errorGeneric").'</h1>
			<form class="form__inner" method="post" action="">
	...
	if ($id > 0) {
		$query = $db->prepare("INSERT INTO gauntlets (id, level1, level2, level3, level4, level5, timestamp) VALUES (:id, :l1, :l2, :l3, :l4, :l5, :t)");
		$query->execute([':id' => ExploitPatch::number($id), ':l1' => ExploitPatch::remove($_POST["level_1"]), ':l2' => ExploitPatch::remove($_POST["level_2"]), ':l3' => ExploitPatch::remove($_POST["level_3"]), ':l4' => ExploitPatch::remove($_POST["level_4"]), ':l5' => ExploitPatch::remove($_POST["level_5"]), ':t' => time()]);
	} else {
	...
	'<option value="-1">' . $dl->getLocalizedString("gauntletSelectAutomatic") . '</option>';
	$gauntletOptions = '<option value="-1">' . $dl->getLocalizedString("gauntletSelectAutomatic") . '</option>';
	for ($x = 1; $x <= $gs->getGauntletCount(); $x++) {
		$query = $db->prepare('SELECT * FROM gauntlets WHERE id = :id');
		$query->execute(['id' => $x]);
		if ($query->rowCount() < 1) {
			$gauntletOptions .= '<option value="' . $x . '">' . $gs->getGauntletName($x) . ' Gauntlet</option>';
		}
	}
	...
	<div class="field" id="iddiv" style="grid-gap:5px">
	<label for="gauntlet_id">' . $dl->getLocalizedString("gauntletSelect") . '</label>
		<select id="gauntlet_id" name="gauntlet_id">
			' . $gauntletOptions . '
		</select>
	</div>
