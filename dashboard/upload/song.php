	...
	if(!$uploadSong['success']) {
		switch($uploadSong['error']) {
			case SongError::Banned:
				exit(Dashboard::renderToast("gavel", sprintf(Dashboard::string("bannedToast"), Escape::url_base64_decode($uploadSong['info']['reason']), '<text dashboard-date="'.$uploadSong['info']['expires'].'"></text>'), "error"));
			...
			case SongError::TooBig:
				exit(Dashboard::renderToast("xmark", sprintf(Dashboard::string("errorMaxFileSize"), $maxSongSize), "error"));
...
$dataArray = ['UPLOAD_SONG_BUTTON_ONCLICK' => "handleSongUpload('uploadSongForm')"];
